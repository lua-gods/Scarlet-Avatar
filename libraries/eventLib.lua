...
function eventLib:remove(name)
   for _, priority in pairs(self.priorities) do
      if self.listensers[priority][name] then
         self.listensers[priority][name] = nil
      end
   end
end
...
---@param func any
---@param priority any
function eventLib:register(func,name,priority)
   ...
   if type(name) == "string" then
      self.listensers[priority][name] = func
      return Listener.new(priority,name,self)
   else
